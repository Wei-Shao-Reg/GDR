...
using namespace std;
const unsigned int Dimension = 3; // image dimenstion = 3
...
int main(int argc, char *argv[]) {
    ...
    std::vector<float> scale_LBFGS; // step size used in LBFGS optimizer
    ...
    if (cFile.is_open())
    {
        ...
        while(getline(cFile, line)){
            ...
            {
                std::stringstream ss(value);
                file << "scale_LBFGS = " + value + "\n\n";
                float i;
                while (ss >> i)
                {
                    scale_LBFGS.push_back(i);
                    if (ss.peek() == ',')
                        ss.ignore();
                }
            }
    ...
    imageSizeInitial[2] = imageSizeInitial[2]/initialsampleFactor;
    ...
    imageSpacingInitial[2] = imageSpacingInitial[2]*initialsampleFactor;
       ...
       // for(int i =1; i <= numberOfTimeIntervals; i++)
        ...
        if (similarityCostType=="SSTVD") // if tissue density image is used for regression
        {
           // while (  iter<numberOfIterations.at(level-1) && ( (((cost_pre - cost_cur)/cost_pre)<0 || cost_pre==cost_cur || ((cost_pre - cost_cur)/cost_pre)>epsilons.at(level-1)) || iter<=2) )
            while (  iter<numberOfIterations.at(level-1) && ( convergeRate > epsilons.at(level-1) || iter<=2) && scale/scale_LBFGS[level-1]>0.01 )
            {
                ...
                for(int l=0; l< numberOfInputs - 1; l++)
                {
                    ...
                    {
                    // deform x,y,z components of v[i] by u[i], u[i+1] = u[i] + v[i](Id + u[i])*intervalLength
                    ...
                    // select the x, y, and z compoments of velocityField[i]
                    ...
                    IndexSelectionType::Pointer indexSelectionFilterZ = IndexSelectionType::New();
                    indexSelectionFilterZ->SetIndex(2); // 2 for z direction
                    indexSelectionFilterZ->SetInput(velocityField[i]);
                    indexSelectionFilterZ->Update();
                    
                    //// deform x,y,z components of v[i] by u[i]
                    ...
                    ResampleImageFilterType::Pointer   resampleZ = ResampleImageFilterType::New();
                    resampleZ->SetInput(indexSelectionFilterZ->GetOutput());
                    resampleZ->SetSize( imageSizeDown );
                    resampleZ->SetOutputOrigin(  inputImages[0]->GetOrigin() );
                    resampleZ->SetOutputSpacing( imageSpacingDown );
                    resampleZ->SetOutputDirection( inputImages[0]->GetDirection() );
                    resampleZ->SetTransform(dispTransform);
                    resampleZ->SetDefaultPixelValue(0);
                    resampleZ->Update();
                    
                    /// mutiply deformed vector field v[i] by interval length
                    ...
                    multiplyZByIntervalLength->SetInput(resampleZ->GetOutput());
                    ...
                    composeImageFilter->SetInput(2,multiplyZByIntervalLength->GetOutput());
                ...
                for(int l=0; l< numberOfInputs - 1; l++)
                {
                    ...
                    for(int i = inputImageTimes.at(l); i< inputImageTimes.at(l+1); i++)
                    {
                        ...
                        IndexSelectionType::Pointer VZ = IndexSelectionType::New();
                        VZ->SetIndex(2); // 2 for z direction
                        VZ->SetInput(velocityField[i]);
                        VZ->Update();
                        ...
                        IndexSelectionType::Pointer UZ = IndexSelectionType::New();
                        UZ->SetIndex(2); // 2 for z direction
                        UZ->SetInput(dispFieldLagrangian[i]);
                        UZ->Update();
                        ...
                        DerivativeFilterType::Pointer       UXzDerivative = DerivativeFilterType::New();
                        UXzDerivative->SetInput(UX->GetOutput());
                        UXzDerivative->SetDirection(2);
                        UXzDerivative->Update();
                        ...
                        DerivativeFilterType::Pointer       UYzDerivative = DerivativeFilterType::New();
                        UYzDerivative->SetInput(UY->GetOutput());
                        UYzDerivative->SetDirection(2);
                        UYzDerivative->Update();
                        
                        DerivativeFilterType::Pointer       UZxDerivative = DerivativeFilterType::New();
                        UZxDerivative->SetInput(UZ->GetOutput());
                        UZxDerivative->SetDirection(0);
                        UZxDerivative->Update();
                        
                        DerivativeFilterType::Pointer       UZyDerivative = DerivativeFilterType::New();
                        UZyDerivative->SetInput(UZ->GetOutput());
                        UZyDerivative->SetDirection(1);
                        UZyDerivative->Update();
                        
                        DerivativeFilterType::Pointer       UZzDerivative = DerivativeFilterType::New();
                        UZzDerivative->SetInput(UZ->GetOutput());
                        UZzDerivative->SetDirection(2);
                        UZzDerivative->Update();
                        ...
                        ResampleImageFilterType::Pointer   resample13 = ResampleImageFilterType::New();
                        resample13->SetInput(UXzDerivative->GetOutput());
                        resample13->SetSize( imageSizeDown );
                        resample13->SetOutputOrigin(  inputImages[0]->GetOrigin() );
                        resample13->SetOutputSpacing( imageSpacingDown );
                        resample13->SetOutputDirection( inputImages[0]->GetDirection() );
                        resample13->SetTransform(dispTransform);
                        resample13->SetDefaultPixelValue(0);
                        resample13->Update();
                        ImageType::Pointer D13 = resample13->GetOutput();
                        ...
                        ResampleImageFilterType::Pointer   resample23 = ResampleImageFilterType::New();
                        resample23->SetInput(UYzDerivative->GetOutput());
                        resample23->SetSize( imageSizeDown );
                        resample23->SetOutputOrigin(  inputImages[0]->GetOrigin() );
                        resample23->SetOutputSpacing( imageSpacingDown );
                        resample23->SetOutputDirection( inputImages[0]->GetDirection() );
                        resample23->SetTransform(dispTransform);
                        resample23->SetDefaultPixelValue(0);
                        resample23->Update();
                        ImageType::Pointer D23 = resample23->GetOutput();
                        
                        
                        ResampleImageFilterType::Pointer   resample31 = ResampleImageFilterType::New();
                        resample31->SetInput(UZxDerivative->GetOutput());
                        resample31->SetSize( imageSizeDown );
                        resample31->SetOutputOrigin(  inputImages[0]->GetOrigin() );
                        resample31->SetOutputSpacing( imageSpacingDown );
                        resample31->SetOutputDirection( inputImages[0]->GetDirection() );
                        resample31->SetTransform(dispTransform);
                        resample31->SetDefaultPixelValue(0);
                        resample31->Update();
                        ImageType::Pointer D31 = resample31->GetOutput();
                        
                        ResampleImageFilterType::Pointer   resample32 = ResampleImageFilterType::New();
                        resample32->SetInput(UZyDerivative->GetOutput());
                        resample32->SetSize( imageSizeDown );
                        resample32->SetOutputOrigin(  inputImages[0]->GetOrigin() );
                        resample32->SetOutputSpacing( imageSpacingDown );
                        resample32->SetOutputDirection( inputImages[0]->GetDirection() );
                        resample32->SetTransform(dispTransform);
                        resample32->SetDefaultPixelValue(0);
                        resample32->Update();
                        ImageType::Pointer D32 = resample32->GetOutput();
                        
                        ResampleImageFilterType::Pointer   resample33 = ResampleImageFilterType::New();
                        resample33->SetInput(UZzDerivative->GetOutput());
                        resample33->SetSize( imageSizeDown );
                        resample33->SetOutputOrigin(  inputImages[0]->GetOrigin() );
                        resample33->SetOutputSpacing( imageSpacingDown );
                        resample33->SetOutputDirection( inputImages[0]->GetDirection() );
                        resample33->SetTransform(dispTransform);
                        resample33->SetDefaultPixelValue(0);
                        resample33->Update();
                        ImageType::Pointer D33 = resample33->GetOutput();
                        ...
                        itk::ImageRegionIterator<ImageType> iterator13(D13,D13->GetBufferedRegion());
                        ...
                        itk::ImageRegionIterator<ImageType> iterator23(D23,D23->GetBufferedRegion());
                        itk::ImageRegionIterator<ImageType> iterator31(D31,D31->GetBufferedRegion());
                        itk::ImageRegionIterator<ImageType> iterator32(D32,D32->GetBufferedRegion());
                        itk::ImageRegionIterator<ImageType> iterator33(D33,D33->GetBufferedRegion());
                        iterator11.GoToBegin();
                        iterator12.GoToBegin();
                        iterator13.GoToBegin();
                        iterator21.GoToBegin();
                        iterator22.GoToBegin();
                        iterator23.GoToBegin();
                        iterator31.GoToBegin();
                        iterator32.GoToBegin();
                        iterator33.GoToBegin();
                        while(!iterator11.IsAtEnd())
                        {
                            // Get the current pixel
                            float a = 1+ iterator11.Get();
                            float b = iterator12.Get();
                            float c = iterator13.Get();
                            float d = iterator21.Get();
                            float e = 1+ iterator22.Get();
                            ...
                            iterator32.Set((b*g - a*h)/det);
                            iterator33.Set((a*e - b*d)/det);
                        ...
                        MultiplyImageType::Pointer         multiplyXZ =  MultiplyImageType::New();
                        multiplyXZ->SetInput1(D13);
                        multiplyXZ->SetInput2(VZ->GetOutput());
                        multiplyXZ->Update();
                        ...
                        AddImageType::Pointer    addX2 = AddImageType::New();
                        addX2->SetInput1(addX1->GetOutput());
                        addX2->SetInput2(multiplyXZ->GetOutput());
                        addX2->Update();
                        
                      //  AddImageType::Pointer    addX3 = AddImageType::New();
                      //  addX3->SetInput1(addX2->GetOutput());
                     //   addX3->SetInput2(VX->GetOutput());
                     //   addX3->Update();
                        ...
                        MultiplyImageType::Pointer         multiplyYZ =  MultiplyImageType::New();
                        multiplyYZ->SetInput1(D23);
                        multiplyYZ->SetInput2(VZ->GetOutput());
                        multiplyYZ->Update();
                        ...
                        AddImageType::Pointer    addY2 = AddImageType::New();
                        addY2->SetInput1(addY1->GetOutput());
                        addY2->SetInput2(multiplyYZ->GetOutput());
                        addY2->Update();
                        
                    //    AddImageType::Pointer    addY3 = AddImageType::New();
                    //    addY3->SetInput1(addY2->GetOutput());
                    //    addY3->SetInput2(VY->GetOutput());
                    //    addY3->Update();
                        
                        
                        
                        MultiplyImageType::Pointer         multiplyZX =  MultiplyImageType::New();
                        multiplyZX->SetInput1(D31);
                        multiplyZX->SetInput2(VX->GetOutput());
                        multiplyZX->Update();
                        
                        MultiplyImageType::Pointer         multiplyZY =  MultiplyImageType::New();
                        multiplyZY->SetInput1(D32);
                        multiplyZY->SetInput2(VY->GetOutput());
                        multiplyZY->Update();
                        
                        MultiplyImageType::Pointer         multiplyZZ =  MultiplyImageType::New();
                        multiplyZZ->SetInput1(D33);
                        multiplyZZ->SetInput2(VZ->GetOutput());
                        multiplyZZ->Update();
                        
                        AddImageType::Pointer    addZ1 = AddImageType::New();
                        addZ1->SetInput1(multiplyZX->GetOutput());
                        addZ1->SetInput2(multiplyZY->GetOutput());
                        addZ1->Update();
                        
                        AddImageType::Pointer    addZ2 = AddImageType::New();
                        addZ2->SetInput1(addZ1->GetOutput());
                        addZ2->SetInput2(multiplyZZ->GetOutput());
                        addZ2->Update();
                        
                    //    AddImageType::Pointer    addZ3 = AddImageType::New();
                    //    addZ3->SetInput1(addZ2->GetOutput());
                    //    addZ3->SetInput2(VZ->GetOutput());
                   //     addZ3->Update();
				...
				if (iter >= M + 2)
                {
					...
					for (int i=0; i<=numberOfTimeIntervals; i++)
                    {
                        ...
                        DerivativeFilterType::Pointer       zDerivative = DerivativeFilterType::New();
                        ...
                        yDerivative->Update();
                        zDerivative->SetInput(costateImages[i]);
                        zDerivative->SetDirection(2);
                        zDerivative->Update();
                        ...
                        MultiplyImageType::Pointer      multiplyZComponent = MultiplyImageType::New();
                        multiplyZComponent->SetInput1(stateImages[i]);
                        multiplyZComponent->SetInput2(zDerivative->GetOutput());
                        multiplyZComponent->Update();
                        ImageType::Pointer     zProd = multiplyZComponent->GetOutput();
                        ...
                        GaussianFilterType::Pointer    filterZProd  =  GaussianFilterType::New();
                        filterZProd->SetInput(zProd);
                        filterZProd->SetSigma(myKernelSize);
                        filterZProd->Update();
                        
                        ImageToVectorImageFilterType::Pointer       composeImageFilter =  ImageToVectorImageFilterType::New();
                        composeImageFilter->SetInput(0,filterXProd->GetOutput());
                        composeImageFilter->SetInput(1,filterYProd->GetOutput());
                        composeImageFilter->SetInput(2,filterZProd->GetOutput());
                        composeImageFilter->Update();
        ...
        if (similarityCostType=="SSD")
        {
           // while (  iter<numberOfIterations.at(level-1) && ( (((cost_pre - cost_cur)/cost_pre)>epsilons.at(level-1)) || iter<=20) && abs(myStepSize)>0.000001 )
            while (  iter<numberOfIterations.at(level-1) && ( convergeRate > epsilons.at(level-1) || iter<=2) && abs(myStepSize)>0.000001 )
            {
                /// solve map flow forward
                for(int l=0; l< numberOfInputs - 1; l++)
                {
                ...
                if (cost_cur<cost_pre||iter==1)
                {
                    for (int i=0; i<=numberOfTimeIntervals; i++)
                    {
                        ...
                        yDerivative->SetDirection(1);
                        yDerivative->Update();
                        zDerivative->SetInput(stateImages[i]);
                        zDerivative->SetDirection(2);
                        zDerivative->Update();
                        ...
                        MultiplyImageType::Pointer      multiplyZComponent = MultiplyImageType::New();
                        multiplyZComponent->SetInput1(costateImages[i]);
                        multiplyZComponent->SetInput2(zDerivative->GetOutput());
                        multiplyZComponent->Update();
                        ImageType::Pointer     zProd = multiplyZComponent->GetOutput();
   ...
   //         ImageWriterType::Pointer ImageWriter = ImageWriterType::New();
    ...
    for(int l=0; l< numberOfInputs - 1; l++)
    {
        ...
        for(int i = inputImageTimes.at(l); i< inputImageTimes.at(l+1); i++)
        {
            ...
            ResampleImageFilterType::Pointer   resample13 = ResampleImageFilterType::New();
            resample13->SetInput(UXzDerivative->GetOutput());
            resample13->SetSize( imageSize );
            resample13->SetOutputOrigin(  inputImages[0]->GetOrigin() );
            resample13->SetOutputSpacing( imageSpacing );
            resample13->SetOutputDirection( inputImages[0]->GetDirection() );
            resample13->SetTransform(dispTransformEulerian);
            resample13->SetDefaultPixelValue(0);
            resample13->Update();
            ImageType::Pointer D13 = resample13->GetOutput();
            ...
            ResampleImageFilterType::Pointer   resample23 = ResampleImageFilterType::New();
            resample23->SetInput(UYzDerivative->GetOutput());
            resample23->SetSize( imageSize );
            resample23->SetOutputOrigin(  inputImages[0]->GetOrigin() );
            resample23->SetOutputSpacing( imageSpacing );
            resample23->SetOutputDirection( inputImages[0]->GetDirection() );
            resample23->SetTransform(dispTransformEulerian);
            resample23->SetDefaultPixelValue(0);
            resample23->Update();
            ImageType::Pointer D23 = resample23->GetOutput();
            ...
            ResampleImageFilterType::Pointer   resample31 = ResampleImageFilterType::New();
            resample31->SetInput(UZxDerivative->GetOutput());
            resample31->SetSize( imageSize );
            resample31->SetOutputOrigin(  inputImages[0]->GetOrigin() );
            resample31->SetOutputSpacing( imageSpacing );
            resample31->SetOutputDirection( inputImages[0]->GetDirection() );
            resample31->SetTransform(dispTransformEulerian);
            resample31->SetDefaultPixelValue(0);
            resample31->Update();
            ImageType::Pointer D31 = resample31->GetOutput();
            ...
            ResampleImageFilterType::Pointer   resample32 = ResampleImageFilterType::New();
            resample32->SetInput(UZyDerivative->GetOutput());
            resample32->SetSize( imageSize );
            resample32->SetOutputOrigin(  inputImages[0]->GetOrigin() );
            resample32->SetOutputSpacing( imageSpacing );
            resample32->SetOutputDirection( inputImages[0]->GetDirection() );
            resample32->SetTransform(dispTransformEulerian);
            resample32->SetDefaultPixelValue(0);
            resample32->Update();
            ImageType::Pointer D32 = resample32->GetOutput();
            ...
            ResampleImageFilterType::Pointer   resample33 = ResampleImageFilterType::New();
            resample33->SetInput(UZzDerivative->GetOutput());
            resample33->SetSize( imageSize );
            resample33->SetOutputOrigin(  inputImages[0]->GetOrigin() );
            resample33->SetOutputSpacing( imageSpacing );
            resample33->SetOutputDirection( inputImages[0]->GetDirection() );
            resample33->SetTransform(dispTransformEulerian);
            resample33->SetDefaultPixelValue(0);
            resample33->Update();
            ImageType::Pointer D33 = resample33->GetOutput();
            // .....end deform components of Dphi[i]
            ...
            composeImageFilterEulerian->SetInput(2,multiplyZByIntervalLengthEulerian->GetOutput());
            ...
            ResampleImageFilterType::Pointer   resampleZ = ResampleImageFilterType::New();
            resampleZ->SetInput(indexSelectionFilterZ->GetOutput());
            resampleZ->SetSize( imageSize );
            resampleZ->SetOutputOrigin(  inputImages[0]->GetOrigin() );
            resampleZ->SetOutputSpacing( inputImages[0]->GetSpacing() );
            resampleZ->SetOutputDirection( inputImages[0]->GetDirection() );
            resampleZ->SetTransform(dispTransform);
            resampleZ->SetDefaultPixelValue(0);
            resampleZ->Update();
    ...
    while(!weightImageIterator.IsAtEnd())
    {
        ...
        if (fixedTemplate=="False")
        {
            if (similarityCostType=="SSTVD")
            {
               ...
               // DivideImageFilterType::Pointer divideImageFilter = DivideImageFilterType::New ();
               // divideImageFilter->SetInput1(templateImageFinal);
               // divideImageFilter->SetInput2(weightImageFinal);
               // divideImageFilter->Update();
               // templateImageFinal = divideImageFilter-> GetOutput();
    ...
    /// compute flow of diffeomorphisms from time point inputImageTimes.at(1) (0EX) to all other phases
    /// t goes from 0 to 1 on a continuum, we decretize this into numberOfTimeIntervals = 55 time points for an example
    /// we also have the number of phases = numberOfInputs, where this goes from 0 to numberOfInputs-1,
    /// where input 0 is time 0, input 1 is time 5, etc. thus inputImageTimes.at(0) = 0, inputImageTimes.at(1) = 5
    /// currently, we are computing map flow starting from the second input (l=1), in the future, we user should be able to specify the starting point
    for(int l=outputDispStartTime; l< numberOfInputs - 1; l++) /////// need to change this in the future....................................................
    {
        VectorFieldImageType::Pointer  dispCurrentPush = VectorFieldImageType::New();
        dispCurrentPush = dispFieldPush[l];
